...
const state = {
  ...
  recipes: [],
  categories: [],
  meals: new Map(),      // "YYYY-MM-DD|slot|eater" -> { id, recipe_id }
  picker: null           // { date, slot, eater }
};
...
async function loadRecipes() {
  const { data, error } = await state.sb
    .from('recipes')
    .select('id, name, target, category, prep_minutes, ingredients, notes')
...
async function loadMeals() {
  ...
  const { data, error } = await state.sb
    .from('meals')
    .select('id, meal_date, slot, eater, recipe_id')
    .gte('meal_date', from)
    .lte('meal_date', to);
  if (error) throw error;
  state.meals = new Map();
  (data || []).forEach((m) => {
    state.meals.set(key(m.meal_date, m.slot, m.eater), { id: m.id, recipe_id: m.recipe_id });
  });
}
...
/* =============================================================
   ...
   ============================================================= */
function openPicker(date, slot, eater) {
  state.picker = { date, slot, eater };
...
function renderPickerList() {
  ...
  const eater = state.picker.eater;
  const allowed = eater === 'bimbo' ? ['bimbo', 'entrambi'] : ['adulti', 'entrambi'];
  ...
  if (!list.length) {
    el.innerHTML = `<p class="empty">Nessuna ricetta compatibile. Aggiungila dalla pagina <b>Ricette</b>
      (tag «${eater === 'bimbo' ? 'Per bimbi' : 'Per adulti'}» o «Entrambi»).</p>`;
    return;
  }
...
async function assignMeal(recipeId) {
  ...
  if (!p) return;
  const payload = { meal_date: p.date, slot: p.slot, eater: p.eater, recipe_id: recipeId };
  const { data, error } = await state.sb
    .from('meals')
  ...
  state.meals.set(key(p.date, p.slot, p.eater), { id: row ? row.id : null, recipe_id: recipeId });
  banner('');
  closePicker();
  renderRecap();
}
...
async function clearMeal() {
  ...
  state.meals.delete(key(p.date, p.slot, p.eater));
  banner('');
  closePicker();
  renderRecap();
}
...
function wire() {
  ...
  $('#calendar').addEventListener('click', (e) => {
    ...
    openPicker(s.dataset.date, s.dataset.slot, s.dataset.eater);
  });
  ...
  ['#r-search', '#r-filter-target', '#r-filter-category'].forEach((sel) => {
